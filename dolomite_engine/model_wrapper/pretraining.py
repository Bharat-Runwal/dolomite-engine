...
class ModelWrapperForPretraining(ModelWrapper):
    ...
    def forward(
        ...
        Args:
            ...
            is_multimodal (bool): Multimodal training 
        Returns:
        ...
        if is_multimodal :
            output = self.forward_multimodal(batch,aux_loss_from_pipeline_parallel,lm_loss_multiplier)
    ...
    def forward_multimodal(
        ...
        """Multimodal forward function for a batch
        ...
        batch = self._prepare_model_inputs_multimodal(batch)
        labels = batch.pop("labels")
        output: CausalLMOutputWithPast | PipelineParallelOutput = self.model(**batch, return_dict=True)
    ...
    def get_mm_loss(
        ...
        # Get the lm_loss and use loss_mask on top of it 
    ...
    def _prepare_model_inputs_multimodal(self, batch: dict) -> dict:
        if self.is_pipeline_parallel_enabled:
            raise NotImplementedError("Pipeline Parallel setup with multimodal pre-training is not supported yet")
        else:
            if ProcessGroupManager.is_tensor_parallel_enabled():
                raise NotImplementedError("Tensor Parallel setup with multimodal pre-training is not supported yet")
            else:
                tokens = batch["text"]
                tokens = tokens.to(torch.cuda.current_device())
                
            input_ids = tokens[:, :-1].contiguous()
            position_ids = batch["position_ids"].to(tokens.device)
            attention_mask = batch["attention_mask"].to(tokens.device)
            loss_mask = batch["loss_mask"].to(tokens.device)
            vision_patch_indices = batch["vision_patch_indices"].to(tokens.device)
            vision_patches = batch["vision_patches"].to(self.dtype).to(tokens.device) #TODO: Recheck data type
            
            batch = {"labels":tokens[:, 1:].contiguous(),
