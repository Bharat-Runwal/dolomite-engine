...
from .multimodal_dataset import MultimodalInstructionDataset
import numpy as np 

DistributedDataset = Union[BlendedDataset, MegatronDataset, MMapIndexedDataset]


class BlendedMegatronMultimodalDatasetBuilder(object):
    ...
    def _build_megatron_dataset_splits(
        ...
        """
        indexed_datasets = self._get_indexed_dataset(path_prefix)

        total_docs = len(indexed_datasets["text"])


        # generate random permutation of documents
        #TODO : Randomize the subset selection here also ? 
        np_rng = np.random.RandomState(seed=self.config.random_seed)
        # docs_indices = np_rng.permutation(total_docs)

        if indexed_datasets is not None:
            split_idx_bounds = _get_split_indices(split, total_docs)
        ...
        for i, _split in enumerate(Split):
            if split[i] == 0.0:
                ...
                megatron_datasets.append(
                    self._sample_dataset(np_rng, split_indices[i], indexed_datasets,
                                             _split.name, sizes[i], self.config.sequence_length)

                )

        return megatron_datasets

    def _sample_dataset(self,np_rng: np.random.RandomState, document_indices: np.ndarray,
                    indexed_datasets, name: str,
        ...
        dataset = self._build_generic_dataset( # TODO :Check This again 
    ...
    def _get_indexed_dataset(self, path_prefix: str) -> MMapIndexedDataset | None:
        if not torch.distributed.is_initialized() or self.config.is_built_on_rank:
            ...
            indexed_text = MMapIndexedDataset(f"{path_prefix}-text", self.cls.is_multimodal())
            indexed_role = MMapIndexedDataset(f"{path_prefix}-role", self.cls.is_multimodal())
            indexed_vision_patch = MMapIndexedDataset(f"{path_prefix}-vision_patch", self.cls.is_multimodal())
            indexed_vision_patch_indices = MMapIndexedDataset(f"{path_prefix}-vision_patch_indices", self.cls.is_multimodal())
