...
from .blended_megatron_mm_dataset_builder import BlendedMegatronMultimodalDatasetBuilder
...
def get_megatron_multimodal_dataloaders(args: TrainingArgs, tokenizer: AutoTokenizer, consumed_samples: int, collator_fn = None) -> None:
    assert len(args.datasets) == 1
    class_args = args.datasets[0].class_args
    ...
    gpt_dataset_builder = BlendedMegatronMultimodalDatasetBuilder(
        sizes=_get_train_val_test_samples(
            args.training_parameters.num_training_steps,
            micro_batch_size,
            gradient_accumulation_steps,
    ...
    # Option 4: data loading using --(train|val|test)-weighted-split-paths
        ...
        raise NotImplementedError("train_weighted_split_paths : Not implemented for multimodal yet")
    else:
        raise NotImplementedError("No dataloading argument passed")

    log_rank_0(logging.INFO, "> finished creating Multimodal datasets ...")

    def _get_dataloader(dataset: MultimodalInstructionDataset | None, consumed_samples: int):
        # we use batch sampler here to match the data order of NVIDIA's megatron repo
        if dispatching_dataloader:
            is_dataset_none_on_source_rank = [dataset is None if is_built_on_rank else False]
            _, _source_rank, _, _broadcast_group = get_source_and_broadcast_group(source_broadcast_mapping)
            ...
            dataloader = DispatchingDataLoader(
                ...
                keys=["text", "role", "vision_patch_indices", "vision_patch"], # TODO: Check this part again 
            ...
            dataloader = ResumableDataLoader(
                dataset, batch_sampler=batch_sampler, num_workers=class_args.get("num_workers", 2), pin_memory=True ,collate_fn = collator_fn
            )
