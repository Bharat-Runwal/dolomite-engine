...
Usage:
    python visualize.py [model_path] [--output_dir DIR] [--num_samples N] [--dataset wikitext]
"""
...
def main():
    ...
    parser.add_argument("model_path", nargs="?",
        default="/proj/dmfexp/energy-gpt/checkpoints-bsaha/unsharded/egpt_test/EGPT_4_blocks_ml_4_14_14_4_30k")
    parser.add_argument("--output_dir", default="tools/bs/energy_landscape")
    ...
    if not all_energies:
        print("No energy blocks found in this model (all softmax_attention). Skipping energy plots.")
    ...
    ax = axes[1, 1]
    total_blocks = max(k[0] for k in all_hidden_norms.keys()) + 1 if all_hidden_norms else num_blocks
    max_norm_iter = max(k[1] for k in all_hidden_norms.keys()) if all_hidden_norms else max_iter
    norm_matrix = np.full((total_blocks, max_norm_iter + 1), np.nan)
    ...
    for block_idx in range(total_blocks):
        iters_x = []
        norms_y = []
        for iter_idx in range(max_norm_iter + 1):
            if not np.isnan(norm_matrix[block_idx, iter_idx]):
                iters_x.append(iter_idx)
                norms_y.append(norm_matrix[block_idx, iter_idx])
        if iters_x:
            c = plt.cm.tab20(block_idx / max(total_blocks - 1, 1))
            ax.plot(iters_x, norms_y, 'o-', label=f'B{block_idx}', color=c,
