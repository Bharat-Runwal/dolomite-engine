"""Combine energy plots for all 7 EGPT_5b_9_9_9_9_9 models (30k-90k) and
...
def combine_existing_plots():
    base = Path("tools/bs/energy_landscape/EGPT_5b_9_9_9_9_9")
    steps = ["30k", "40k", "50k", "60k", "70k", "80k", "90k"]
    ...
    for plot_file, title in plot_types.items():
        ...
        labels = []
        for step in steps:
            p = base / f"EGPT_5b_9_9_9_9_9_{step}" / plot_file
            if p.exists():
                paths.append(p)
        ...
        if paths:
            ...
            combine_vertical(paths, labels, base / out_name,
                           title=f"EGPT_5b_9_9_9_9_9: {title} (30k-90k)")
...
def load_energy_data():
    ...
    # Data structures: step_label -> block_idx -> {iter_idx: [energy_values]}
    all_data = {}

    for step, label in zip(STEPS, STEP_LABELS):
        model_name = f"EGPT_5b_9_9_9_9_9_{label}"
...
def plot_cross_step_figures(all_data, step_labels):
    out_dir = Path("tools/bs/energy_landscape/EGPT_5b_9_9_9_9_9")
    ...
    stats = {}  # step -> block -> dict of stats
    for label in available:
        ...
        for b in range(num_blocks):
            ...
            # Per-iteration energies for detailed trajectory
            iter_energies = []
            ...
            # Convergence: mean relative change across iterations
            deltas = []
    ...
    # A1: Energy delta per block vs steps
    ax = axes[0, 0]
    for b in range(num_blocks):
    ...
    # A2: Convergence rate per block vs steps
    ax = axes[0, 1]
    for b in range(num_blocks):
    ...
    # A3: Initial energy (iter 0) per block vs steps
    ax = axes[0, 2]
    for b in range(num_blocks):
    ...
    # A4: Final energy (last iter) per block vs steps
    ax = axes[1, 0]
    for b in range(num_blocks):
    ...
    # A5: Total energy (sum across blocks) at first and last iter vs steps
    ax = axes[1, 1]
    ...
    # A6: Energy spread (max - min across blocks) at first and last iter
    ax = axes[1, 2]
    ...
    fig.suptitle('EGPT_5b_9_9_9_9_9: Cross-Step Energy Analysis Dashboard\n(wikitext, 30 samples)',
    ...
    fig.suptitle('EGPT_5b_9_9_9_9_9: Energy per (Step, Iteration) for Each Block\n(wikitext, 30 samples)',
    ...
    # =====================================================================
    # FIGURE C: Per-block energy curves overlaid across steps (one subplot per block)
    # =====================================================================
    ...
    fig.suptitle('EGPT_5b_9_9_9_9_9: Energy Curves Across Training Steps\n(wikitext, 30 samples)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # =====================================================================
    # FIGURE D: Energy "efficiency" - how much energy is reduced per iteration
    # =====================================================================
    fig, axes = plt.subplots(1, 2, figsize=(18, 7))

    # D1: Per-iteration energy change for each block at 30k vs 90k
    ax = axes[0]
    bar_width = 0.15
    for b in range(num_blocks):
        for si, (label, marker) in enumerate([(available[0], 'o'), (available[-1], 's')]):
            ...
            if len(ie) > 1:
                ...
                offset = b * bar_width * 0.3
                ax.plot([x + offset for x in iters], deltas, marker + '-',
                       label=f'B{b} @ {label}' if si == 0 or b == 0 else None,
                       color=cmap[b], alpha=0.8 if si == 0 else 0.4,
                       linewidth=2 if si == 1 else 1, linestyle='-' if si == 0 else '--',
                       markersize=5)
    ...
    # D2: Cumulative energy change across iterations for first vs last step
    ax = axes[1]
    for b in range(num_blocks):
        for si, label in enumerate([available[0], available[-1]]):
            ...
            if ie:
                ...
                ax.plot(iters, cum_delta, 'o-' if si == 0 else 's--',
                       label=f'B{b} @ {label}' if b == 0 else None,
    ...
    fig.suptitle('EGPT_5b_9_9_9_9_9: Energy Dynamics Comparison (Early vs Late Training)\n(wikitext, 30 samples)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
