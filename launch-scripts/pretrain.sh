...
#
# Use an ISOLATED venv so the shared .venv (used by other branches) is never
# disturbed. Default targets transformers 4.57.1 (energy model builds cleanly there).
# Override with PRETRAIN_VENV=/path/to/venv if needed (e.g. a kernel-ready 7B env).
_PRETRAIN_REPO_ROOT="/proj/dmfexp/bishwajit/Code/dolomite-engine"
_PRETRAIN_VENV="${PRETRAIN_VENV:-${_PRETRAIN_REPO_ROOT}/.venv-boltz}"
