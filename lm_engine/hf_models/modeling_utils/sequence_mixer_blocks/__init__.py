...
SEQUENCE_MIXER_TYPE = Attention | CausalConvolution | GRU | Mamba2 | MultiHeadLatentAttention | RNN | GatedDeltaNet | EnergyAttention_QK | MixedHeadAttention | EnergyGradMixedHeadAttention
...
def get_sequence_mixer(
    ...
    if sequence_mixer_type == "causal_convolution":
        ...
        return cls(
            hidden_size=config.hidden_size,
            num_attention_heads=block.num_attention_heads,
            num_energy_heads=getattr(block, 'num_energy_heads', block.num_attention_heads // 2),
    ...
    elif sequence_mixer_type in ("energy_attention", "vk_residual"):
        return EnergyAttention_QK(
            ...
            vk_pre_rope=getattr(config, "energy_vk_pre_rope", False),
        )
    else:
        ...
        if sequence_mixer_type in ("softmax_attention", "projected_softmax"):
            return Attention(
