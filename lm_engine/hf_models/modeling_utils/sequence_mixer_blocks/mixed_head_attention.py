...
class EnergyGradMixedHeadAttention(MixedHeadAttention):
    ...
    def forward(
        ...
        # ── KV cache: pack energy K (V=K) alongside GPT K,V ─────────────────
        # Energy heads need cached K_e for generation; store as combined tensor
        K_all = torch.cat([K_e, K_g], dim=1)
        V_all = torch.cat([K_e, V_g], dim=1)

        if past_key_values is not None:
            cache_idx = layer_id if layer_id is not None else self.layer_idx
            K_all, V_all = past_key_values.update(key_states=K_all, value_states=V_all, layer_idx=cache_idx)
        ...
        energy_attn_mask = attention_mask
        if energy_attn_mask is not None:
            if energy_attn_mask.dim() == 2:
                # [B, T_kv] padding mask -> [B, 1, 1, T_kv] for SDPA broadcast
                energy_attn_mask = energy_attn_mask[:, None, None, :].to(Q_e.dtype)
            elif energy_attn_mask.dtype not in (torch.bool, torch.float32, torch.float16, torch.bfloat16):
                energy_attn_mask = energy_attn_mask.to(Q_e.dtype)
        attn_e = F.scaled_dot_product_attention(
            Q_e, K_e, K_e,
            dropout_p=self.softmax_dropout_p if self.training else 0,
            is_causal=self.causal if attention_mask is None else False,
            attn_mask=energy_attn_mask,
            scale=scale,
        )  # (B, ne, T, dh)
        ...
        if use_fa2 or use_fa3:
            ...
            gpt_attn_mask = attention_mask
            if gpt_attn_mask is not None:
                if gpt_attn_mask.dim() == 2:
                    gpt_attn_mask = gpt_attn_mask[:, None, None, :].to(Q_g.dtype)
                elif gpt_attn_mask.dtype not in (torch.bool, torch.float32, torch.float16, torch.bfloat16):
                    gpt_attn_mask = gpt_attn_mask.to(Q_g.dtype)
            attn_g = F.scaled_dot_product_attention(
                Q_g, K_g, V_g,
                attn_mask=gpt_attn_mask,
                dropout_p=self.softmax_dropout_p if self.training else 0,
                is_causal=self.causal if attention_mask is None else False,
                scale=scale,
            )
